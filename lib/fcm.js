...
function FCM(accountKey) {
    if(!accountKey) {
        ...
        this.send = function (payload, options, CB) {
        ...
        this.send = function(payload, options, _callback){
            if (!_callback) {
                    ...
                    if(payload.to) {
                        if (typeof payload.to == 'string') {
                            ...
                            if (to.startsWith('/topics/')) {
                                ...
                                firebaseadmin.messaging().sendToDevice(to, payload, options)
                                    .then(function (response) {_callback(null,response)})
                        ...
                        {
                            firebaseadmin.messaging().sendToDevice(regIds, payload, options)
                                .then(function (response) {_callback(null,response)})
                                .catch(function (error) {_callback(error)})
                        }
                        else{
